...
public class Room {
	...
	public static void printRoomToys (ArrayList<Room> rooms) throws EmptyToyException {
		if (rooms.isEmpty()){
		...
		while(i < count){
			...
			for (String size: sizes){
				...
				for (String name: names){
					Toy newToy = OperationsWithToys.searchToysForRoom(averagePrice, name, size, toys);
			...
			//specify sort criteria
			 ...
			 for (Room room: rooms){
				 ...
				 if (room.getToy().isEmpty()){
					 ...
					 OperationsWithToys.sortToy(room.getToy(), sort);
				 }                   	 
			 }   
			 if (rooms.isEmpty()){
				 throw(new EmptyToyException("Room is empty"));
	...
	public static void searchToysInRooms(ArrayList<Room> rooms) {
		...
		for (Room room: rooms){
			 System.out.println("Room: \t " + room.roomName);
			OperationsWithToys.searchToyByParam(size, price, color, age, material, room.getToy());
		}
	}
}